...
public class XSetting {
	...
	private static void load(Context context) {
		...
		setting = context.getApplicationContext().getSharedPreferences(
				context.getPackageName() + "_preferences", 0);
	}

	public final static long xget_long(Context context, String string) {
		if (setting == null)
			load(context);
		return Long.valueOf(setting.getString(string, "0"));
	}
	...
	public final static float xget_float_super(Context context, String string,
			String s) {
		if (setting == null)
			load(context);
		float i;
	...
	public final static void xset_string_int(Context context, int i, String edit) {
		if (setting == null)
			load(context);
	...
	public final static void xset_boolean(Context context, int i, boolean edit) {
		if (setting == null)
			load(context);
